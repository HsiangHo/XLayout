...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    var bigDemoArray: [(String, String)] = []
...
//        simpleCodeTest()
        ...
        //some simple demo code with tableview cell (self-resizing cellview height):
        cellViewSelfResizingHeightTest()
    }
    ...
    func cellViewSelfResizingHeightTest() {
        for _ in 0...1000 {
            let value: Int = Int(arc4random() % 3)
            bigDemoArray.append(demoArray[value])
        }
...
extension AppDelegate: NSTableViewDelegate, NSTableViewDataSource {
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        if let view = viewCell as? AutolayoutCellView {
            view.title.stringValue = bigDemoArray[row].0
            view.text.stringValue = bigDemoArray[row].1
    ...
    func tableView(_ tableView: NSTableView, heightOfRow row: Int) -> CGFloat {
        return AutolayoutCellView.dynamicHeight(tableView, bigDemoArray[row].0, bigDemoArray[row].1)
    }
}
